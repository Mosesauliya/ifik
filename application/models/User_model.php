...
class User_model extends CI_Model {
    ...
    public function __construct()
    {
        ...
        $this->load->database();
        $this->table = $this->db->table_exists('user') ? 'user' : 'users';
    }
    ...
    {
        $ident = trim($identity);
        $table = !empty($this->table) ? $this->table : 'user';
        return $this->db
            ->select('id, username, name, email, password, role_id, is_active, status, nip, nim, images')
            ->from($table)
            ->group_start()
                ->where('email', $ident)
                ->or_where('username', $ident)
                ->or_where('nim', $ident)
            ->group_end()
            ->limit(1)
            ->get()
    ...
    public function get_by_id($id)
    {
        $table = !empty($this->table) ? $this->table : 'user';
        return $this->db
            ->select('id, username, name, email, role_id, is_active, status, nip, nim, images')
            ->from($table)
            ->where('id', $id)
            ->limit(1)
            ->get()
    ...
    public function get_role_id_by_name($roleName)
    {
        ...
        if (strpos($roleName, 'dosen') !== false) return 3;
        if (strpos($roleName, 'mahasiswa') !== false) return 4;
        if (strpos($roleName, 'laboran') !== false) return 2;
        if (strpos($roleName, 'kaur') !== false || strpos($roleName, 'ka. ur') !== false) return 2;
        ...
        if ($this->db->table_exists('user_role')) {
            $role = $this->db->get_where('user_role', ['LOWER(role)' => $roleName])->row();
            if ($role) return (int)$role->id;
        } elseif ($this->db->table_exists('roles')) {
            $role = $this->db->get_where('roles', ['name' => $roleName])->row();
            if ($role) return (int)$role->id;
        }
        return 4; // Default to Mahasiswa (4)
    }
