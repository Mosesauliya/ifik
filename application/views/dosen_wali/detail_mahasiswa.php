...
<body class="bg-slate-50 text-slate-800 font-sans antialiased min-h-screen flex flex-col selection:bg-orange-600 selection:text-white">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex-grow w-full space-y-6">
        ...
        <?php if($isLocked): ?>
            ...
            <div class="bg-blue-50 border border-blue-200 rounded-2xl p-4 sm:p-5 flex items-start gap-4 shadow-xs">
                <div class="w-10 h-10 rounded-xl bg-blue-600 text-white flex items-center justify-center text-lg font-bold shrink-0 shadow-xs">
        ...
        <div class="clean-card rounded-2xl p-6 sm:p-7 relative overflow-hidden">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        ...
                        <div class="text-xs text-slate-500 flex flex-wrap items-center gap-x-4 gap-y-1">
                            <span><i class="bi bi-mortarboard text-slate-400 mr-1"></i> <?= htmlspecialchars($detail['prodi'] ?? 'Desain Komunikasi Visual'); ?></span>
                ...
                <div class="flex flex-col sm:items-end gap-1.5">
                    <span class="text-[10px] uppercase font-bold tracking-wider text-slate-400">Status Saat Ini di Dosen Wali:</span>
                    <?php if($status_wali === 'Approved'): ?>
                    ...
                    <?php elseif($status_wali === 'Rejected'): ?>
            ...
            <div class="space-y-6">
                ...
                <div class="clean-card rounded-2xl p-6 sm:p-7 space-y-5">
                    <div>
                        ...
                        <textarea id="catatan_admin" name="catatan_wali" rows="3" 
                                    placeholder="Tuliskan catatan perbaikan kepada mahasiswa..."
                                    <?= $isLocked ? 'readonly' : ''; ?>
                                    oninput="updateActionButtonsUI()"
                                    class="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 focus:bg-white transition-all leading-relaxed"><?= htmlspecialchars($detail['catatan_wali'] ?? ''); ?></textarea>
                    </div>
                    ...
                    <div class="pt-4 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4">
                        ...
                        <div class="order-1 sm:order-2">
                            <span class="inline-flex items-center gap-2 px-4 py-2 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-xl text-xs font-bold shadow-xs">
                                <i class="bi bi-shield-check text-sm text-emerald-600"></i> Berkas Telah Disetujui &amp; Dikunci
                            </span>
                        </div>
                        <?php endif; ?>
                    </div>
        ...
        <div class="bg-white rounded-2xl max-w-5xl w-full h-[88vh] flex flex-col overflow-hidden shadow-2xl border border-slate-200">
            ...
            <div class="p-3.5 px-5 bg-slate-900 text-white flex items-center justify-between flex-shrink-0">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <button type="button" onclick="closePdfModal()" class="w-8 h-8 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white font-bold text-lg flex items-center justify-center transition-colors cursor-pointer">&times;</button>
                </div>
            </div>
            ...
            <div class="p-3.5 px-5 bg-white border-t border-slate-200 flex flex-wrap items-center justify-between gap-3 flex-shrink-0">
                <div class="flex items-center gap-2">
                    <button type="button" onclick="simulateDownload()" class="px-3.5 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-colors cursor-pointer">
    ...
    <script>
        ...
        function confirmReject() {
            ...
            if (confirm('Yakin ingin mengembalikan pengajuan ini ke mahasiswa untuk revisi/perbaikan berkas?')) {
