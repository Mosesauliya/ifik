...
<body class="bg-gradient-to-br from-slate-50 via-orange-50/20 to-slate-100 min-h-screen text-slate-800 antialiased">
    ...
    <main class="min-h-screen p-4 sm:p-6 lg:p-10 max-w-7xl mx-auto">
        ...
        <div class="bg-white rounded-2xl sm:rounded-3xl border border-slate-200/80 shadow-sm p-3.5 sm:p-4 mb-6">
            <form action="<?= site_url('adminlayanan/jadwal_sidang'); ?>" method="GET" class="flex items-center gap-2.5">
                <div class="unified-search-pill flex-1 flex items-center justify-between gap-1 min-w-0">
                    ...
                    <input type="text" name="q" value="<?= htmlspecialchars($search ?? ''); ?>" placeholder="Cari nama mahasiswa, NIM, ruangan, atau dosen penguji..." class="w-full text-xs font-semibold bg-transparent border-none focus:outline-none text-slate-800 placeholder:text-slate-400">
                ...
                <table class="w-full text-left border-collapse text-xs">
                    <thead>
                        <tr class="border-b border-slate-100 bg-slate-50/60 text-[11px] font-extrabold text-slate-400 uppercase tracking-wider">
                            ...
                            <th class="py-3.5 px-4">Waktu & Ruangan</th>
                            <th class="py-3.5 px-5">Mahasiswa & NIM</th>
                            <th class="py-3.5 px-5">Judul Tugas Akhir</th>
                            <th class="py-3.5 px-4">Tim Pembimbing</th>
                            <th class="py-3.5 px-4">Tim Penguji</th>
                        ...
                        <?php else: ?>
                            <?php foreach ($list as $r): ?>
                                <tr class="hover:bg-slate-50/80 transition-colors">
                                    ...
                                    <!-- Judul TA -->
                                    <td class="py-4 px-5 text-slate-700 font-semibold leading-relaxed max-w-[220px]">
                                        <?= htmlspecialchars($r['judul']); ?>
                                    </td>

                                    <!-- Pembimbing -->
                                    <td class="py-4 px-4 text-slate-700 space-y-1">
